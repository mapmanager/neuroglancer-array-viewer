...
The direct viewer now consolidates presentation controls into a custom Options menu: checked scale-bar, axis-line, and display-dimensions toggles plus `Fit image`. Fit is not a source reset; it preserves display state, centers XY, and derives zoom from every live panel's dimensions and pinned projection-scale factors. A single full-height Z rail is shown only for multi-plane XY and per-channel layouts. Direct slider input remains smooth JavaScript state, native wheel positions update it bidirectionally, and stacked/side-by-side layouts retain one shared rail outside all panels. The native top-left X/Y/Z element is the display-dimensions widget. The pinned UI configuration exposes no granular visibility flag, so its runtime toggle is an intentionally narrow compatibility exception: a project-scoped `.neuroglancer-display-dimensions-widget` rule verified against the exact pinned DOM and requiring revalidation on any upstream pin change.

The Options menu also independently toggles the project-owned Channels and Layout chrome with standard DOM hidden state. A single native-layout-buttons item controls both classless per-panel buttons through exact, project-scoped title selectors verified as `Switch to 4panel-alt layout.` and `Switch to xy-3d layout.` at the pinned revision. This is the same documented compatibility boundary and must be rechecked with any Neuroglancer upgrade.
